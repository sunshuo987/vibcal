...
from pytreenet.util.misc_functions import orthogonalise_gep, linear_combination, orthogonalise_to
...
def run_block_inverse_iteration(ham_pad: Hamiltonian, ttnss: List[TreeTensorNetworkState], save_to_path: str,num_sweeps = 5, svd_params: SVDParameters = SVDParameters(max_bond_dim=10, rel_tol=1e-5, total_tol=1e-5), file_path: List[str] = []):
    ...
    for i in range(num_sweeps):
        ...
        if len(file_path) > 0:
            ...
            for ttns in ttnss:
                ttns = orthogonalise_to(ttns, file_path, svd_params.max_bond_dim, num_sweeps)
...
def main(results_dir, num_sweeps = 10, max_bond_dim = 12):
    ...
    for ix, cl in enumerate(clusters[0:]):
        ...
        time_end = time.time()
        file_path = [f"{state_dir}/inverse_iteration_state_{i}_wresi_20" for i in range(cl_state_cnt, cl_state_cnt + len(cl))]
        ...
        cl_state_cnt += len(cl)
        np.savetxt(f"{results_dir}/inverse_iteration_energies_cache_20_cl19.csv", np.concatenate(energies,axis = 0), delimiter=",")

    energies = np.concatenate(energies,axis = 0)
    np.savetxt(f"{results_dir}/inverse_iteration_energies_20_cl19.csv", np.array(energies), delimiter=",")
    profiler.disable()
    profiler.dump_stats(f"{results_dir}/profile_output_ii_20_cl19.prof")

if __name__ == "__main__":
    # main(results_dir = "./Results_test/ch3cn/mps/max_bond_dim_12", 
    #      num_sweeps = 5, max_bond_dim = 20)
    # main(results_dir = "./Results_test/ch3cn/threetree/max_bond_dim_12", 
    #      num_sweeps = 5, max_bond_dim = 20)
    main(results_dir = "./Results_test/ch3cn/leafonly/max_bond_dim_12", 
         num_sweeps = 5, max_bond_dim = 20)
