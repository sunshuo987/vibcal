...
def generate_random_matrices(ttns: TTNS,
    ...
    base_dtype = ttns.tensors[root_id].dtype
    is_real_dtype = np.issubdtype(base_dtype, np.floating)
    tensors = {}
    for node_id, node in ttns.nodes.items():
        ...
        if is_real_dtype:
            ...
            rand_tensor = rng.standard_normal(tuple(desired_shape)).astype(base_dtype, copy=False)
        else:
            rand_tensor = crandn(tuple(desired_shape), seed=seed)
            if rand_tensor.dtype != base_dtype:
                rand_tensor = rand_tensor.astype(base_dtype, copy=False)
        copy_t = copy_tensor(node.nlegs(), desired_dimension)
        if copy_t.dtype != base_dtype:
            copy_t = copy_t.astype(base_dtype, copy=False)
